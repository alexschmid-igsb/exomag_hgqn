...
const knex = require('./database/access').init().connection


// Setup async init tasks
const asyncInits = [
    require('./redis/RedisConnection').initPromise,
    require('./user/store').initPromise

    // require('./redis/RedisClient').init(),
    // require('./user/store').init()
]
...
fs.readdirSync(path.join(__dirname, 'routes')).forEach( file => {
    if (file !== 'index.js') {
        ...
        // In dieser App sollen allerdings nur die API endpoints geroutet werden. Das Frontend wird
        // über einen webserver (z.b. nginx) ausgeliefert.
